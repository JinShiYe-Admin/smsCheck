<template>
	<view ref="node" name="LyTreeNode" v-show="node.visible" class="ly-tree-node" :class="{ 
		...
		<view class="ly-tree-node__content" :class="[{
		...
		},node.childNodesId.length==0?'ly-tree-node__content_last':'ly-tree-node__content_noLast']" :style="{ 
				'margin-left': (node.level - 1) * indent + 'px' 
			}">
			...
			<text class="ly-tree-node__label" :class="node.childNodesId.length==0?'ly-tree-node__label_last':'ly-tree-node__label_noLast'">{{node.label}}</text>
		</view>
...
<style>
	...
	.ly-tree-node__content_last {
		
	}

	.ly-tree-node__content_noLast {
		
	}
	.ly-tree-node__label_last{
